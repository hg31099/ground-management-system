...
char a[20];
    ...
    //SORT ACCORDING TO CITYNAME
        for(i=0;i<rec;i++)
        {
          for(j=0;j<rec-1;j++)
          {
          	//	if(strcmp(tfs[j].sportsName[k],tfs[j+1].sportsName[k+1])==0)
    ...
    for(l=0;l<count;l++)
    {
        ...
        for(i=0;i<20;i++)
        {
            for(j=0;j<5;j++)
            {
                ...
                if((strcmp(gs[l].gamename,g[i].sports[j].gameName)==0) && (g[i].bg[k].end-g[i].bg[k].start>=gs[l].timeduration))
                {
                    for(z=gs[l].starttime;z<=gs[l].endtime-gs[l].timeduration && flag==0;z++)
                    {
                        if(z+gs[l].timeduration<=g[i].bg[k].end)
                        {
                            flag=1;
                        }
                    }
                    if(flag==1)
                    {
                        cost=(g[i].sports[k].rate)*(gs[l].timeduration);
                        if(cost<min)
                        {
                            min=cost;
                            index=i;
        ...
        strcpy(gs[l].groundname,g[index].groundName);
    }
    for(k=0;k<count;k++)
    {
        printf("%s can be played in %s ground \n",gs[k].gamename,gs[k].groundname);
;
    }
...
void enterValuesIntoDatabase()
{
	...
	strcpy(g[0].sports[2].gameName,"Badminton");
	strcpy(g[0].sports[3].gameName,"Tabletennis");
	strcpy(g[0].sports[4].gameName,"Football");
		...
		strcpy(g[1].sports[1].d,"20171121"); //in yyyymmdd FORMAT
			strcpy(g[1].sports[2].d,"20170301"); //in yyyymmdd FORMAT
				strcpy(g[1].sports[3].d,"20170801"); //in yyyymmdd FORMAT
					...
					g[1].sports[0].endTime=200; //in tttt FORMAT
	...
	strcpy(g[2].sports[1].gameName,"Swimming");
		strcpy(g[2].sports[2].gameName,"Khokho");
	
	strcpy(g[2].sports[3].gameName,"Shortput");
strcpy(g[2].sports[4].gameName,"Football");
